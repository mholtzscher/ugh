...
func TestParseCreate_Basic(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result, err := antlrparser.Parse(tt.input, nlp.ParseOptions{})
...
func TestParseCreate_WithDates(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result, err := antlrparser.Parse(tt.input, nlp.ParseOptions{Now: now})
			require.NoError(t, err, "Parse error")
			cmd := result.Command.(*nlp.CreateCommand)
...
func TestParseCreate_WithState(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "state inbox", input: "add task state:inbox", wantState: "inbox"},
		{name: "state now", input: "add task state:now", wantState: "now"},
		{name: "state waiting", input: "add task state:waiting", wantState: "waiting"},
		{name: "state later", input: "add task state:later", wantState: "later"},
		{name: "state done", input: "add task state:done", wantState: "done"},
		{name: "state todo", input: "add task state:todo", wantState: "todo"},
	}
...
func TestParseCreate_WithWaiting(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "waiting simple", input: "add task waiting:alex", wantWaiting: "alex"},
		{name: "multiple words", input: "add task waiting:the team", wantWaiting: "the team"},
	}
...
func TestParseCreate_WithNotes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "simple notes", input: "add task notes:remember this", wantNotes: "remember this"},
		{name: "notes with special chars", input: "add task notes:call before 5pm", wantNotes: "call before 5pm"},
	}
...
func TestParseUpdate_Targets(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "selected target", input: "set selected state:now", wantKind: nlp.TargetSelected, wantID: 0},
		{name: "numeric ID target", input: "set 123 state:now", wantKind: nlp.TargetID, wantID: 123},
		{name: "hash ID target", input: "set #123 state:now", wantKind: nlp.TargetID, wantID: 123},
		{name: "edit verb with ID", input: "edit 456 title:new title", wantKind: nlp.TargetID, wantID: 456},
		{name: "update verb with ID", input: "update 789 state:done", wantKind: nlp.TargetID, wantID: 789},
	}
...
func TestParseUpdate_SetOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "set title", input: "set selected title:new title",
			wantField: nlp.FieldTitle, wantValue: "new title",
		},
		{
			name: "set notes", input: "set selected notes:updated notes",
			wantField: nlp.FieldNotes, wantValue: "updated notes",
		},
		{
			name: "set due", input: "set selected due:tomorrow",
			wantField: nlp.FieldDue, wantValue: "tomorrow",
		},
		{
			name: "set waiting", input: "set selected waiting:john",
			wantField: nlp.FieldWaiting, wantValue: "john",
		},
		{
			name: "set state", input: "set selected state:done",
			wantField: nlp.FieldState, wantValue: "done",
		},
	}
...
func TestParseUpdate_AddOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "add project", input: "set selected +project:work",
			wantField: nlp.FieldProjects, wantValue: "work",
		},
		{
			name: "add context", input: "set selected +context:urgent",
			wantField: nlp.FieldContexts, wantValue: "urgent",
		},
		{
			name: "add meta", input: "set selected +meta:priority:high",
			wantField: nlp.FieldMeta, wantValue: "priority:high",
		},
	}
...
func TestParseUpdate_RemoveOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "remove project", input: "set selected -project:old",
			wantField: nlp.FieldProjects, wantValue: "old",
		},
		{
			name: "remove context", input: "set selected -context:waiting",
			wantField: nlp.FieldContexts, wantValue: "waiting",
		},
	}
...
func TestParseUpdate_ClearOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "clear due", input: "set selected !due", wantField: nlp.FieldDue},
		{name: "clear waiting", input: "set selected !waiting", wantField: nlp.FieldWaiting},
		{name: "clear notes", input: "set selected !notes", wantField: nlp.FieldNotes},
	}
...
func TestParseUpdate_Combined(t *testing.T) {
	...
	result, err := antlrparser.Parse(`set selected state:now +project:work !due`, nlp.ParseOptions{})
	require.NoError(t, err, "Parse error")
	cmd := result.Command.(*nlp.UpdateCommand)
...
func TestParseFilter_Basic(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "find verb", input: "find state:now", wantIntent: nlp.IntentFilter},
		{name: "show verb", input: "show #work", wantIntent: nlp.IntentFilter},
		{name: "filter verb", input: "filter state:waiting", wantIntent: nlp.IntentFilter},
		{name: "list verb", input: "list project:personal", wantIntent: nlp.IntentFilter},
	}
...
func TestParseFilter_Predicates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "state predicate", input: "find state:now", wantKind: nlp.PredState, wantText: "now"},
		{name: "project predicate", input: "find project:work", wantKind: nlp.PredProject, wantText: "work"},
		{name: "context predicate", input: "find context:phone", wantKind: nlp.PredContext, wantText: "phone"},
		{name: "project tag shorthand", input: "find #work", wantKind: nlp.PredProject, wantText: "work"},
		{name: "context tag shorthand", input: "find @phone", wantKind: nlp.PredContext, wantText: "phone"},
		{name: "text predicate", input: "find text:report", wantKind: nlp.PredText, wantText: "report"},
		{name: "id predicate explicit", input: "find id:42", wantKind: nlp.PredID, wantText: "42"},
		{name: "id predicate numeric", input: "find 42", wantKind: nlp.PredID, wantText: "42"},
		{name: "show numeric is id lookup", input: "show 3", wantKind: nlp.PredID, wantText: "3"},
		{name: "show hash numeric is id lookup", input: "show #3", wantKind: nlp.PredID, wantText: "3"},
	}
...
func TestParseFilter_LogicalAnd(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "and keyword", input: "find state:now and project:work"},
		{name: "double ampersand", input: "find state:now && project:work"},
	}
...
func TestParseFilter_LogicalOr(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "or keyword", input: "find state:now or state:waiting"},
		{name: "double pipe", input: "find state:now || state:waiting"},
	}
...
func TestParseFilter_LogicalNot(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "not keyword", input: "find not state:done"},
		{name: "bang operator", input: "find ! state:done"},
	}
...
func TestParseFilter_PrecedenceAndParentheses(t *testing.T) {
	...
	t.Run("and has higher precedence than or", func(t *testing.T) {
		t.Parallel()
		result, err := antlrparser.Parse("find state:now or state:waiting and project:work", nlp.ParseOptions{})
		require.NoError(t, err, "Parse error")
		cmd := result.Command.(*nlp.FilterCommand)
...
func TestParseErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "invalid update target", input: "set banana state:now"},
		{name: "empty field value", input: "add task state:"},
		{name: "invalid tag token", input: "add task #"},
		{name: "unterminated quoted string", input: `add task "email #hashtag`},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := antlrparser.Parse(tt.input, nlp.ParseOptions{})
...
func TestParseUpdate_TagShorthand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "set project tag shorthand", input: "set #work",
			wantTarget: nlp.TargetSelected, wantTagKind: nlp.TagProject, wantTagValue: "work",
		},
		{
			name: "set context tag shorthand", input: "set @urgent",
			wantTarget: nlp.TargetSelected, wantTagKind: nlp.TagContext, wantTagValue: "urgent",
		},
		{
			name: "edit with tag shorthand", input: "edit #personal",
			wantTarget: nlp.TargetSelected, wantTagKind: nlp.TagProject, wantTagValue: "personal",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd, ok := result.Command.(*nlp.UpdateCommand)
			require.True(t, ok, "command type should be UpdateCommand, got %T", result.Command)
			assert.Equal(t, tt.wantTarget, cmd.Target.Kind, "target kind mismatch")
			...
			assert.Equal(t, tt.wantTagKind, tagOp.Kind, "tag kind mismatch")
			assert.Equal(t, tt.wantTagValue, tagOp.Value, "tag value mismatch")
		})
	}
}
...
func TestParseFilterCommand(t *testing.T) {
	...
	result, err := antlrparser.Parse(`find state:now and project:work`, nlp.ParseOptions{})
...
func TestParseViewAndContextErrors(t *testing.T) {
	...
	_, err := antlrparser.Parse("view maybe", nlp.ParseOptions{})
	require.Error(t, err, "expected parse error for invalid view")

	_, err = antlrparser.Parse("context maybe", nlp.ParseOptions{})
...
func TestParseModeViewAndContext(t *testing.T) {
	...
	result, err := antlrparser.Parse("view inbox", nlp.ParseOptions{Mode: nlp.ModeView})
	...
	result, err = antlrparser.Parse("context #work", nlp.ParseOptions{Mode: nlp.ModeContext})
...
func TestParseInvalidUpdateTarget(t *testing.T) {
	...
	_, err := antlrparser.Parse(`set banana state:now`, nlp.ParseOptions{})
...
func TestLex_Basic(t *testing.T) {
	...
	tokens, err := antlrparser.Lex("add buy milk #groceries @store")
	require.NoError(t, err, "Lex error")

	// Should produce: Ident(add), Ident(buy), Ident(milk), ProjectTag(#groceries), ContextTag(@store)
	require.GreaterOrEqual(t, len(tokens), 5, "expected at least 5 tokens, got %d", len(tokens))

	assert.Equal(t, "Ident", tokens[0].Name)
	assert.Equal(t, "add", tokens[0].Value)

	// Find project tag
	foundProject := false
	for _, tok := range tokens {
		if tok.Name == "ProjectTag" && tok.Value == "#groceries" {
			foundProject = true
			break
		}
	}
	assert.True(t, foundProject, "expected ProjectTag token for #groceries")
}

func TestLex_PositionOffsets(t *testing.T) {
	t.Parallel()

	//                        0123456789...
	tokens, err := antlrparser.Lex("add buy milk #groceries")
	require.NoError(t, err, "Lex error")
	require.GreaterOrEqual(t, len(tokens), 4)

	// "add" starts at offset 0
	assert.Equal(t, 0, tokens[0].Pos.Offset, "first token offset")
	assert.Equal(t, "add", tokens[0].Value)

	// "buy" starts at offset 4
	assert.Equal(t, 4, tokens[1].Pos.Offset, "second token offset")
	assert.Equal(t, "buy", tokens[1].Value)

	// "milk" starts at offset 8
	assert.Equal(t, 8, tokens[2].Pos.Offset, "third token offset")
	assert.Equal(t, "milk", tokens[2].Value)

	// "#groceries" starts at offset 13
	assert.Equal(t, 13, tokens[3].Pos.Offset, "fourth token offset")
	assert.Equal(t, "#groceries", tokens[3].Value)
}
